...
```ts
const body: V1RecurringsRequest = {
  interval: 1,
...
try {
  const { result, ...httpResponse } = await recurringController.createANewRecurringRecord(body);
...
| `fields` | [`Field40Enum[] \| undefined`](../../doc/models/field-40-enum.md) | Query, Optional | You can use any `field_name` from this endpoint results to filter the list of fields returned on the response. |
| `requestOptions` | `RequestOptions \| undefined` | Optional | Pass additional request options. |
...
try {
  const { result, ...httpResponse } = await recurringController.listAllRecurringRecord(
  page,
  order,
  filterBy
);
...
```ts
const recurringId = '11e95f8ec39de8fbdb0a4f1a';

try {
  const { result, ...httpResponse } = await recurringController.deleteRecurringRecord(recurringId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await recurringController.viewSingleRecurringRecord(recurringId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
const body: V1RecurringsRequest1 = {
...
try {
  const { result, ...httpResponse } = await recurringController.updateRecurringPayment(
  recurringId,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await recurringController.activateRecurringPayment(recurringId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
const body: V1RecurringsDeferPaymentRequest = {
  deferCount: 5,
};

try {
  const { result, ...httpResponse } = await recurringController.deferRecurringPayment(
  recurringId,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await recurringController.placeOnHoldRecurringPayment(recurringId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
const body: V1RecurringsSkipPaymentRequest = {
  skipCount: 7,
};

try {
  const { result, ...httpResponse } = await recurringController.skipRecurringPayment(
  recurringId,
  body
);
  // Get more response info...
