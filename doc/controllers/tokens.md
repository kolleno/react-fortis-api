...
try {
  const { result, ...httpResponse } = await tokensController.createANewACHToken(body);
...
try {
  const { result, ...httpResponse } = await tokensController.createANewCreditCardToken(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
```ts
const body: V1TokensPreviousTransactionRequest = {
  locationId: '11e95f8ec39de8fbdb0a4f1a',
  previousTransactionId: '11e95f8ec39de8fbdb0a4f1a',
  accountHolderName: 'John Smith',
  accountVaultApiId: 'accountvaultabcd',
...
try {
  const { result, ...httpResponse } = await tokensController.createANewPreviousTransactionToken(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
```ts
const body: V1TokensTerminalAsyncRequest = {
  locationId: '11e95f8ec39de8fbdb0a4f1a',
  action: 'store',
...
try {
  const { result, ...httpResponse } = await tokensController.createANewTerminalTokenWithAsyncMethod(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
```ts
const body: V1TokensTerminalRequest = {
  locationId: '11e95f8ec39de8fbdb0a4f1a',
  action: 'store',
  terminalId: '11e95f8ec39de8fbdb0a4f1a',
  accountHolderName: 'John Smith',
...
try {
  const { result, ...httpResponse } = await tokensController.createANewTerminalToken(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
```ts
const body: V1TokensTicketRequest = {
  locationId: '11e95f8ec39de8fbdb0a4f1a',
  ticket: '12345678',
  accountHolderName: 'John Smith',
  accountVaultApiId: 'accountvaultabcd',
...
try {
  const { result, ...httpResponse } = await tokensController.createANewTicketToken(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
```ts
const body: V1TokensWalletRequest = {
  locationId: '11e95f8ec39de8fbdb0a4f1a',
...
try {
  const { result, ...httpResponse } = await tokensController.createANewWalletToken(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await tokensController.deleteASingleTokenRecord(tokenId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
| `tokenId` | `string` | Template, Required | A unique, system-generated identifier for the Token.<br>**Constraints**: *Pattern*: `^(([0-9a-fA-F\-]{24,36})\|(([0-9a-fA-F]{8})-(([0-9a-fA-F]{4}\-){3})([0-9a-fA-F]{12})))$` |
| `expand` | [`Expand44Enum[] \| undefined`](../../doc/models/expand-44-enum.md) | Query, Optional | Most endpoints in the API have a way to retrieve extra data related to the current record being retrieved. For example, if the API request is for the accountvaults endpoint, and the end user also needs to know which contact the token belongs to, this data can be returned in the accountvaults endpoint request.<br>**Constraints**: *Unique Items Required*, *Pattern*: `^[\w]+$` |
| `fields` | [`Field50Enum[] \| undefined`](../../doc/models/field-50-enum.md) | Query, Optional | You can use any `field_name` from this endpoint results to filter the list of fields returned on the response. |
| `requestOptions` | `RequestOptions \| undefined` | Optional | Pass additional request options. |
...
try {
  const { result, ...httpResponse } = await tokensController.viewSingleTokenRecord(tokenId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await tokensController.listAllTokensRelated(
  page,
  order,
  filterBy
);
...
const body: V1TokensAchRequest1 = {
  accountHolderName: 'John Smith',
  accountVaultApiId: 'accountvaultabcd',
  tokenApiId: 'tokenabcd',
  accountvaultC1: 'accountvault custom 1',
...
try {
  const { result, ...httpResponse } = await tokensController.updateACHToken(
  tokenId,
  body
);
  // Get more response info...
...
try {
  const { result, ...httpResponse } = await tokensController.updateCCToken(
  tokenId,
  body
);
  // Get more response info...
