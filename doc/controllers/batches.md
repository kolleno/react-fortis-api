...
```ts
const batchesController = new BatchesController(client);
```
...
| `fields` | [`Field26Enum[] \| undefined`](../../doc/models/field-26-enum.md) | Query, Optional | You can use any `field_name` from this endpoint results to filter the list of fields returned on the response. |
