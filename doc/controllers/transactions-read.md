...
```ts
const transactionsReadController = new TransactionsReadController(client);
```
...
[`ResponseTransactionBinInfo`](../../doc/models/response-transaction-bin-info.md)
...
try {
  const { result, ...httpResponse } = await transactionsReadController.getBINInfo(transactionId);
...
| `fields` | [`Field54Enum[] \| undefined`](../../doc/models/field-54-enum.md) | Query, Optional | You can use any `field_name` from this endpoint results to filter the list of fields returned on the response. |
| `requestOptions` | `RequestOptions \| undefined` | Optional | Pass additional request options. |
...
try {
  const { result, ...httpResponse } = await transactionsReadController.getTransaction(transactionId);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await transactionsReadController.listTransactions(
  page,
  order,
  filterBy
);
