...
```ts
const transactionsCashController = new TransactionsCashController(client);
```
...
```ts
const body: V1TransactionsCashRefundRequest = {
  transactionAmount: 1,
...
try {
  const { result, ...httpResponse } = await transactionsCashController.cashRefund(body);
...
```ts
const body: V1TransactionsCashSaleRequest = {
  transactionAmount: 1,
  checkinDate: '2021-12-01',
  checkoutDate: '2021-12-01',
  clerkNumber: 'AE1234',
...
try {
  const { result, ...httpResponse } = await transactionsCashController.cashSale(body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch (error) {
  if (error instanceof ApiError) {
